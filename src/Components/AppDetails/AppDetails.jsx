...
const AppDetails = () => {
    ...
    const [isinstalled, setIsInstalled] = useState(false)// disable and enable state

    useEffect(() => {
        const isInstalled = localStorage.getItem(`installed${id}`) === 'true';
    ...
    const { name, image, title, size, companyName, description, ratings, ratingAvg, downloads, reviews } = clickedApp || {};
        ...
        <div className='container mx-auto p-5'>
            <div className='flex  gap-8 flex-col md:flex-row  '>
                ...
                <div className='flex flex-col flex-1 max-h-[350px] gap-3'>
                    <h2 className='text-[#001931] text-3xl font-bold'><span className='capitalize'>{name}</span> : {title}</h2>
                    ...
                    <button disabled={isinstalled ? true : false} onClick={() => handleInstall(id)} className='btn bg-gradient-to-r from-[#54CF68] to-[#00827A] max-w-[240px] text-white '>{isinstalled ? <span>Installed </span> : <span>Install Now ({size}MB)</span>}</button>
                </div>
            </div>
